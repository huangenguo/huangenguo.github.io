...
1. **需求定义**：明确数据集的用途（如"训练图像分类模型"）和目标。
2. **数据采集**：通过爬虫、传感器、公开数据库（如Kaggle、UCI）等方式获取原始数据。
3. **数据清洗**：处理缺失值、重复值、异常值，统一格式。
4. **数据标注**：人工或自动标注标签（如用LabelImg标注图像）。
5. **特征工程**：提取或构造有效特征，提升模型性能。
6. **数据集划分**：按比例拆分为训练集、验证集、测试集。
7. **数据存储**：保存为常见格式（如CSV、JSON、TFRecord、HDF5）。
